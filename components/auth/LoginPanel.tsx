      ...
      <div className="w-full max-w-md bg-surface-container-lowest rounded-xl shadow-premium p-8 relative animate-fade-in-up">
        ...
        {admin ? (
          <p className="mt-6 font-body-sm text-secondary">
            Community member? <Link href="/login" className="text-accent-magenta font-label-lg">User Sign In</Link>
          </p>
        ) : (
          <p className="mt-6 font-body-sm text-secondary">
            New to SheRides?{" "}
            <Link href={email.trim() ? `/signup?email=${encodeURIComponent(email.trim())}` : "/signup"} className="text-accent-magenta font-label-lg">
              Join Community
            </Link>
          </p>
        )}
