...
export type ChatBubbleMessage = {
  id: string; fromMe: boolean; text: string; time: string; reactions: MessageReaction[];
  audioUrl?: string | null; audioDuration?: number | null;
};
...
function groupReactions(reactions: MessageReaction[], myId: string) {
  ...
  for (const row of reactions) {
    ...
    current.count += 1; if (row.userId === myId) current.mine = true; grouped.set(row.emoji, current);
  }
...
export function MessageBubble({ message, currentUserId, barOpen, onToggleBar, onToggleReaction }: {
  message: ChatBubbleMessage; currentUserId: string; barOpen: boolean; onToggleBar: () => void; onToggleReaction: (emoji:string)=>void;
}) {
  const counts = groupReactions(message.reactions, currentUserId);
  return <div className={`flex ${message.fromMe ? "justify-end" : "justify-start"} animate-fade-in-up`}>
    <div className="group relative max-w-[85%]">
      <div className={`absolute bottom-full mb-1 z-10 flex gap-0.5 px-1 py-1 rounded-full border border-surface-border bg-surface-container-lowest shadow-premium overflow-x-auto ${message.fromMe?"right-0":"left-0"} ${barOpen?"opacity-100 pointer-events-auto":"opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto"}`}>
        {REACTION_EMOJIS.map(emoji=><button key={emoji} type="button" onClick={()=>onToggleReaction(emoji)} className="h-8 w-8 rounded-full hover:bg-surface-container-high">{emoji}</button>)}
      </div>
      <div onClick={onToggleBar} className={`max-w-full p-3 rounded-2xl cursor-pointer ${message.fromMe?"bg-primary-container text-on-primary-container rounded-br-none":"bg-surface-container-lowest border border-surface-border rounded-bl-none"}`}>
        {message.audioUrl ? <div className="min-w-[220px]">
      ...
      {counts.length>0?<div className={`flex flex-wrap gap-1 mt-1 ${message.fromMe?"justify-end":"justify-start"}`}>{counts.map(item=><button key={item.emoji} type="button" onClick={()=>onToggleReaction(item.emoji)} className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[12px] border border-surface-border bg-surface-container-lowest"><span>{item.emoji}</span><span>{item.count}</span></button>)}</div>:null}
    </div>
  </div>;
}
